  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md glass-panel border-l border-zinc-200 rounded-none rounded-l-3xl p-6 flex flex-col justify-between bg-white shadow-2xl animate-in slide-in-from-right duration-300">
          
          {/* Header */}
          <div className="flex items-center justify-between pb-4 border-b border-zinc-200">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-black" />
              <h2 className="font-heading font-black text-xl text-black">Your Cart</h2>
              <span className="badge-neon bg-black text-white border-black text-xs">
                {cartItems.reduce((acc, i) => acc + i.quantity, 0)} items
              </span>
            </div>
            <button
              onClick={onClose}
              className="p-2 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-600 hover:text-black hover:bg-zinc-200"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
            ...
            {cartItems.length === 0 ? (
              ...
              cartItems.map((item) => (
                <div
                  ...
                  className="glass-panel p-3.5 rounded-xl border-zinc-200 bg-zinc-50 flex items-center gap-3"
                >
                  <img
                    src={item.image}
                    alt={item.name}
                    className="w-16 h-16 object-cover rounded-lg bg-white border border-zinc-200 shrink-0"
                  />
                  
                  <div className="flex-1 min-w-0">
                    <h4 className="font-heading font-bold text-sm text-black truncate">
                      {item.name}
                    </h4>
                    ...
                    <div className="font-heading font-bold text-sm text-black mt-1">
